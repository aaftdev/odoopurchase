...
class MaterialPurchaseRequisition(models.Model):
    ...
    def confirm_requisition(self):
        for requisition in self:
            ...
            template_id = self.env['ir.model.data']._xmlid_lookup('bi_material_purchase_requisitions.email_employee_purchase_requisition')[2]
            email_template_obj = self.env['mail.template'].sudo().browse(template_id)
            if template_id:
                values = email_template_obj.generate_email(requisition.id, ['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'])
    ...
    def department_approve(self):
        for requisition in self:
            ...
            template_id = self.env['ir.model.data']._xmlid_lookup('bi_material_purchase_requisitions.email_manager_purchase_requisition')[2]
            email_template_obj = self.env['mail.template'].sudo().browse(template_id)
            if template_id:
                values = email_template_obj.generate_email(requisition.id,['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'])
    ...
    def action_approve(self):
        for requisition in self:
            ...
            template_id = self.env['ir.model.data']._xmlid_lookup('bi_material_purchase_requisitions.email_user_purchase_requisition')[2]
            email_template_obj = self.env['mail.template'].sudo().browse(template_id)
            if template_id:
                values = email_template_obj.generate_email(requisition.id,['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'])
                values['email_from'] = self.employee_id.work_email
